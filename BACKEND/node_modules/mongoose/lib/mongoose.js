 ...
 * uses to communicate with the config. A driver is a Mongoose-specific interface that defines functions
 * like `find()`.
 *
 * @deprecated
 * @memberOf Mongoose
 ...
 *
 * Each `connection` instance maps to a single config. This method is helpful when managing multiple db connections.
 *
 *
 ...
 *
 *     // with mongodb:// URI
 *     db = mongoose.createConnection('mongodb://user:pass@127.0.0.1:port/config');
 *
 *     // and options
 *     const opts = { db: { native_parser: true }}
 *     db = mongoose.createConnection('mongodb://user:pass@127.0.0.1:port/config', opts);
 *
 *     // replica sets
 *     db = mongoose.createConnection('mongodb://user:pass@127.0.0.1:port,anotherhost:port,yetanother:port/config');
 *
 *     // and options
 *     const opts = { replset: { strategy: 'ping', rs_name: 'testSet' }}
 *     db = mongoose.createConnection('mongodb://user:pass@127.0.0.1:port,anotherhost:port,yetanother:port/config', opts);
 *
 *     // initialize now, connect later
 *     db = mongoose.createConnection();
 *     await db.openUri('mongodb://127.0.0.1:27017/config');
 *
 ...
 * @param {String} [options.dbName] The name of the config you want to use. If not provided, Mongoose uses the config name from connection string.
 ...
 * #### Example:
 *
 *     mongoose.connect('mongodb://user:pass@127.0.0.1:port/config');
 *
 *     // replica sets
 ...
 * @param {String} [options.dbName] The name of the config we want to use. If not provided, use config name from connection string.
 * @param {String} [options.user] username for authentication, equivalent to `options.auth.user`. Maintained for backwards compatibility.
 * @param {String} [options.pass] password for authentication, equivalent to `options.auth.password`. Maintained for backwards compatibility.
