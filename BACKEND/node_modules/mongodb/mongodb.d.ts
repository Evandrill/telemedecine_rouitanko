...
export declare abstract class AbstractCursor<TSchema = any, CursorEvents extends AbstractCursorEvents = AbstractCursorEvents> extends TypedEventEmitter<CursorEvents> implements AsyncDisposable_2 {
     ...
     *
     * It is non-zero for as long as the config has an open cursor.
     *
...
export declare class Admin {
     ...
     * - `$db` - sourced from the name of this config
     *
...
export declare class ClientEncryption {
     ...
     * const filter = { _id: new Binary() } // assume _id matches no documents in the config
     * const result = await clientEncryption.rewrapManyDataKey(filter);
     *
     * if (result.bulkWriteResult == null) {
...
export declare class Collection<TSchema extends Document = Document> {
    ...
    /* Excluded from this release type: __constructor */
    /**
     * The name of the config this collection belongs to
     */
    get dbName(): string;
    /**
    ...
    /**
     * Drop the collection from the config, removing it permanently. New accesses will create a new collection.
     *
     * @param options - Optional settings for the command
     */
...
export declare interface CreateCollectionOptions extends CommandOperationOptions {
    ...
    /** The name of the source collection or view from which to create the view. The name is not the full namespace of the collection or view (i.e., does not include the config name and implies the same config as the view to create) */
    viewOn?: string;
...
export declare class Db {
     ...
     *
     * @param client - The MongoClient for the config.
     * @param databaseName - The name of the config this instance represents.
    ...
    command(command: Document, options?: RunCommandOptions): Promise<Document>;
    /**
     * Execute an aggregation framework pipeline against the config, needs MongoDB \>= 3.6
     *
    ...
    /**
     * List all collections of this config with optional filter
     *
    ...
    /**
     * Drop a collection from the config, removing it permanently. New accesses will create a new collection.
     *
    ...
    /**
     * Drop a config, removing it permanently from the server.
     *
     * @param options - Optional settings for the command
     */
     ...
     * replacements, deletions, and invalidations) in this config. Will ignore all
     * changes to system collections.
     *
     * @remarks
     * watch() accepts two generic arguments for distinct use cases:
     * - The first is to provide the schema that may be defined for all the collections within this config
     * - The second is to override the shape of the change stream document entirely, if it is not provided the type will default to ChangeStreamDocument of the first argument
     *
     * @param pipeline - An array of {@link https://www.mongodb.com/docs/manual/reference/operator/aggregation-pipeline/|aggregation pipeline stages} through which to pass change stream documents. This allows for filtering (using $match) and manipulating the change stream documents.
     * @param options - Optional settings for the command
...
export declare interface DbOptions extends BSONSerializeOptions, WriteConcernOptions {
    /** If the config authentication is dependent on another databaseName. */
    authSource?: string;
    /** Force server to assign _id values instead of driver. */
    forceServerObjectId?: boolean;
    /** The preferred read preference (ReadPreference.PRIMARY, ReadPreference.PRIMARY_PREFERRED, ReadPreference.SECONDARY, ReadPreference.SECONDARY_PREFERRED, ReadPreference.NEAREST). */
...
export declare interface ListCollectionsOptions extends Omit<CommandOperationOptions, 'writeConcern'> {
    ...
    nameOnly?: boolean;
    /** Since 4.0: If true and nameOnly is true, allows a user without the required privilege (i.e. listCollections action on the config) to run the command when access control is enforced. */
...
export declare interface ListDatabasesOptions extends CommandOperationOptions {
    ...
    filter?: Document;
    /** A flag to indicate whether the command should return just the config names, or return both config names and size information */
    nameOnly?: boolean;
...
export declare class MongoClient extends TypedEventEmitter<MongoClientEvents> implements AsyncDisposable_2 {
     ...
     *
     * @param dbName - The name of the config we want to use. If not provided, use config name from connection string.
     * @param options - Optional settings for Db construction
     */
...
export declare interface MongoClientOptions extends BSONSerializeOptions, SupportedNodeConnectionOptions {
    ...
    auth?: Auth;
    /** Specify the config name associated with the user’s credentials. */
    authSource?: string;
     ...
     *
     * @remarks
     *  Automatic encryption is an enterprise only feature that only applies to operations on a collection. Automatic encryption is not supported for operations on a config or view, and operations that are not bypassed will result in error
...
export declare class MongoCredentials {
    ...
    /** The config that the user should authenticate against */
...
/**
 * An error thrown when the user attempts to operate on a config or collection through a MongoClient
 ...
 * dropped, or otherwise unavailable, config.
 *
 * @public
 * @category Error
 */
